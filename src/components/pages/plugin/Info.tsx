...
import { Button, theme as antdTheme, Typography, Descriptions } from 'antd'
  ...
  return (
    <div style={{ flex: 1, paddingTop: '30', display: 'flex', flexDirection: 'column', overflowX: 'hidden', width: '100%' }}>
      <Descriptions
        ...
        size="small"
      ></Descriptions>
      {(item?.basicPrompts?.length as number) > 0 && (item?.advancedPrompts?.length as number) > 0 && (
        <Descriptions title="Prompt" column={1} bordered size="small" labelStyle={{ width: 120 }} contentStyle={{ width: 'auto' }}>
          <Descriptions.Item label="Basic Prompt">
            {item?.basicPrompts?.map((item) => {
              return (
                <Typography.Paragraph copyable key={item} style={{ fontSize: 12, marginLeft: '0', marginBottom: '5px' }}>
                  {item}
                </Typography.Paragraph>
              )
            })}
          </Descriptions.Item>
          <Descriptions.Item label="Advanced Prompt">
            {item?.advancedPrompts?.map((item) => {
              return (
                <Typography.Paragraph copyable key={item} style={{ fontSize: 12, marginLeft: '0', marginBottom: '5px' }}>
                  {item}
                </Typography.Paragraph>
      ...
      <Descriptions title="Info" column={1} bordered size="small" style={{ marginTop: 10 }} labelStyle={{ width: 120 }} contentStyle={{ width: 'auto' }}>
        ...
        <Descriptions.Item label="Website">
          <Button type="link" href={item?.website} target={'_blank'} style={{ paddingLeft: 0 }}>
            {item?.website}
          </Button>
        </Descriptions.Item>
        <Descriptions.Item label="ApiUrl">
          <Button type="link" href={item?.apiurl} target={'_blank'} style={{ paddingLeft: 0 }}>
            {item?.apiurl}
          </Button>
        </Descriptions.Item>
