...
function Edit(props: { action: string; prompt?: Prompt }) {
  ...
  const { delPrompt } = usePromptContext()
        ...
        <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', flexDirection: 'column', marginLeft: 10 }}>
            <Typography.Paragraph style={{ fontSize: 16, width: '100%', fontWeight: 500, color: theme === 'dark' ? '#eee' : undefined, margin: 0 }}>
              {action == 'edit' ? prompt?.name : t('prompt.newPrompt')}
            </Typography.Paragraph>
            {action == 'edit' ? (
              <Typography.Paragraph style={{ fontSize: 12, width: '100%', color: theme === 'dark' ? '#eee' : undefined, margin: 0 }}>{prompt?.description || prompt?.prompt}</Typography.Paragraph>
            ) : null}
          </div>
        </div>
        <Space>
